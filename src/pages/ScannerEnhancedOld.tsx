...
export default function ScannerEnhanced() {
  ...
  const performScan = async (type: "camera" | "upload") => {
    ...
    try {
      ...
      if (scanResult.ecoScore >= 80) {
        setShowConfetti(true);
        setNotification({
          title: "🌟 Excellent Choice!",
          message: `This product has a high sustainability score of ${scanResult.ecoScore}%`,
          type: "success"
        });
        setTimeout(() => setShowConfetti(false), 3000);
      } else if (scanResult.ecoScore >= 60) {
        setNotification({
          title: "✅ Good Choice",
          message: `Decent eco-score of ${scanResult.ecoScore}%. Check alternatives for better options.`,
          type: "info"
        });
      } else {
        setNotification({
          title: "⚠️ Consider Alternatives",
          message: `Low eco-score of ${scanResult.ecoScore}%. We found ${alternatives.length} better options.`,
          type: "warning"
        });
      }
                      ...
                      >
                        <div className="text-center">
                          <motion.div
                            ...
                            className="w-16 h-16 mx-auto mb-4 border-4 border-primary border-t-transparent rounded-full"
                          />
                          <p className="text-primary font-medium">Analyzing product...</p>
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-muted/20 rounded-lg p-4 space-y-2">
                      ...
                      <ul className="text-sm text-muted-foreground space-y-1">
                        ...
                        <li>• Focus on product labels and barcodes</li>
                ...
                <Card className="glass-card">
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-muted-foreground">Avg Eco Score</span>
                        <Badge className="bg-green-500/10 text-green-500">78%</Badge>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-muted-foreground">CO₂ Saved</span>
                        <span className="font-semibold text-green-500">15.2 kg</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
